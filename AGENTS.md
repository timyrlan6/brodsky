...
**Задача-максимум** — реконструкция поэтической онтологии Бродского (**собиратель образов**: строки → образы → квинтэссенция → качества → поле → связи концептов). Истина — по Бродскому (мир как фон, приоритет у корпуса). Система предлагает, человек валидирует; два раздельных ручных действия («Найти вхождения» / «Обновить квинтэссенцию»), автономного цикла нет. Частотная подсветка — это этапы 0–1. Полная концепция, обоснования и реестр отвергнутого — `docs/CONTEXT.md` §6a; этапы — §8.
...
## План реализации (этапы; приоритеты предварительны — подробно `docs/CONTEXT.md` §8)
0. Импорт FB2 → PostgreSQL (7 томов), словоформы, частоты, трассировка до строк — детерминированно
1. Плоские концепты (словоформы ядра, вариант B), частотная подсветка, чтение, страница концепта
2. Квинтэссенция: «Обновить» (система предлагает → человек редактирует), версии/diff
3. Поле: «Найти вхождения» (система предлагает → человек подтверждает), вероятностно
4. Знание о мире + отношения концептов, композиционный вывод (кресло = стул + перила)
5. Override (меридианы: корпус правит мир); (после max) интертекст
